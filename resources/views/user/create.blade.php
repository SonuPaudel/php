@extends('layouts.app')
@section('content')
<h2 class="text-4xl font-bold border-b-4 text-red-500 border-black">Users New User</h2>
...
<form action="{{ route('user.store') }}" method="POST" class="my-10">
    ...
    @enderror
    <input type="text" class="w-full p-2 rounded-lg mt-2" name="email" placeholder=" Enter Email " value="{{old('email')}}">
    @error('email')
    <span class="text-red-500 -mt-4">*{{$message}}</span>
    @enderror
    <input type="password" class="w-full p-2 rounded-lg mt-2" name="password" placeholder="Password " value="{{old('password')}}">
    @error('password')
    <span class="text-red-500 -mt-4">*{{$message}}</span>
    @enderror
