@extends('layouts.app')
@section('content')
<h2 class="text-4xl font-bold border-b-4 text-red-500 border-black">Edit Notice</h2>

<form action="{{ route('notice.update',$notice->id) }}" method="POST" class="my-10">
    @csrf
    <input type="text" class="w-full p-2 rounded-lg mt-2" name="notice" placeholder="Notices" value="{{$notice->notice}}">
    @error('notice')
