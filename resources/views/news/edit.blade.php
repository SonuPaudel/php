@extends('layouts.app')
@section('content')
<h2 class="text-4xl font-bold border-b-4 text-red-500 border-blue-500">Edit News</h2>
...
<form action="{{route('news.update',$news->id)}}" method="POST" class="my-10" enctype="multipart/form-data">
    ...
    <input type="text" class="w-full p-2 rounded-lg mt-2" name="news_date" placeholder="Date" value="{{$news->news_date}}">
    @error('news_date')
    ...
    <p>Current Image: </p>
    <img class="w-44" src="{{asset('images/news/'.$news->photopath)}}" alt="">
    ...
    <div class="mt-2">
        ...
        <a href="{{route('news.index')}}" class="bg-red-600 text-white px-4 py-1.5 rounded-lg cursor-pointer">Exit</a>
    </div>
</form>
